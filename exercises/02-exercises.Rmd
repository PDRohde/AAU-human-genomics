...
#### Exercise 15
In the <tt>R</tt>-exercise we will investigate how population size can influence the fixation and extinction probabilities of individual alleles.

The function below `sim` allow you to see how generation time, number of independent populations, and starting allele frequencies affect the probability to loose alleles.
```{r}
...
# Parameters
  ...
  generations <- 100
  num_populations <- 1
  initial_allele_freqs <- c(0.5,0.5, 0.5) 

  tmp <- run_sim(population_sizes=population_sizes, 
                 generations=generations, 
                 num_populations=num_populations, 
                 initial_allele_freqs=initial_allele_freqs)

# Count how many populations that has been fixed  
  tmp$fixed <- paste(tmp$population_size,":",tmp$population_num,sep="")
  is.fixed <- NULL
  for(pop in unique(tmp$fixed)){
    is.fixed <- c(is.fixed, any(tmp[tmp$fixed==pop,]$freq==0 | tmp[tmp$fixed==pop,]$freq==1))
  }
```
<br />
<br />
Make a plot to visualise the results.
```{r, fig.align='center', fig.height=4,fig.width=5}
  ggplot(tmp, aes(x = generation, y = freq, 
